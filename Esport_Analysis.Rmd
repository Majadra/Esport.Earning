...
ggplot(data = HistData %>%
         group_by(Year) %>%
         summarise(EarningsInMillions = 
                     ifelse(sum(Earnings) <= 100*10^6,round(sum(Earnings)/10^6,1),
                            round(sum(Earnings)/10^6,0))) %>%
         mutate(Condition = ifelse(Year == 2020,TRUE,FALSE)),
       aes(x = Year, y = EarningsInMillions, fill = Condition)) +
  geom_bar(stat = "Identity",color = "black",alpha = 0.75) +
  geom_text(aes(label = EarningsInMillions), vjust = -0.3, size = 3) +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(),
        plot.caption = element_text(size = 10)) + 
  labs(title = "Earnings per year*", subtitle = "In millions of dollars", caption = "*not including data from oct 2020 and onwards", y = element_blank(),x = element_blank()) +
...
ggplot(data = HistData %>%
         ...
         summarise(TotalEarnings = sum(Earnings), Tournaments = sum(Tournaments)) %>%
          mutate(Condition = ifelse(Year == 2020,TRUE,FALSE)),
       aes(x = Year, y = Tournaments, fill = Condition)) +
  geom_bar(stat = "identity",color = "black",alpha = 0.75) +
  geom_text(aes(label = Tournaments), vjust = - 0.3, size = 3) +
  labs(title = "Tournaments per year*", y = element_blank(),x = element_blank(), caption = "*not including data from oct 2020 and onwards") +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), 
        plot.caption = element_text(size = 10)) +
  scale_fill_manual(values = c("lightblue","lightblue4")) +
  guides(fill = "none")
...
GenData[order(GenData$TotalPlayers, decreasing = TRUE),] %>%
  slice(1:10) %>%
  ggplot(data = .,aes(x = reorder(Game,+ TotalPlayers), y = TotalPlayers)) + 
  geom_bar(stat = "identity", 
           fill = rainbow(n = 10 , start = 0.45, end = 0.50), color = "black", alpha = 0.85) + 
  coord_flip() +
...
ggplot(data = Genre_plot,
  ...
  labs(title = "Total prizepool earnings by genre*",
       subtitle = "in millions of dollars",caption ="*not including the puzzle genre",
       y = element_blank(),
       x = element_blank(),
       fill = "Genre")
...
ggplot(data = Genre_plot,
  ...
  geom_text(aes(label = Number),
            vjust = -0.3,hjust = 0.55, size = 3.5) +
  theme(axis.text= element_blank(), axis.ticks = element_blank(), legend.position = "right",plot.subtitle = element_text(size = 10)) + 
  labs(title = "Games by genre*",
       caption = "*not including the puzzle genre",
       y = element_blank(),
       x = element_blank(),
       fill = "Genre")
...
```
<br>
It is no surprise that we find **FPS**, **Fighting**, **Racing** and **Sports** as the genres with the most games, it is due to the fact that there are more franchises in these genres, and every franchise has new games being released quite often (the **Call of Duty** and **Fifa** franchises come to mind). <br>
...
ggplot(data = GenData %>%
       ...
       aes(x = factor(Genre,levels = c("Strategy","Fighting","Racing","Sports","FPS")), 
...
ggplot(data = Genre_plot,
  ...
  labs(title = "Average prizepool earnings by genre*",
       subtitle = "in millions of dollars", caption = "*not including the puzzle genre",
       y = element_blank(), x = element_blank(), fill = "Genre")
...
ggplot(data = HistData %>% 
       ...
       aes(x = factor(Year), y = Earnings,fill = Earnings)) +
...
ggplot(data = HistData %>% 
  ...
  geom_vline(xintercept = as.Date("2020-03-11"), linetype = "dotted", color = "darkred",size = 1)
...
We have learn a few things about the Esport industry
